...
1. **Capture a place.** Create a palace and give it a name and a spot capacity
   ...
   - **In the Simulator / on devices without AR / as a fallback**, capture uses a
     **room photo**: choose a picture, then tap spots on it to drop pins.
2. **Pick a card per spot.** Tapping a spot opens a searchable picker over your
   existing MCAT cards (Anki query syntax, e.g. `tag:biochem`). The chosen card
   is pinned there.
...
Sources/AnkiMCAT/
  ...
  Palace/
    ...
    PalaceStudyView.swift ← study session (recall/locate/mixed) → FSRS grading
```
